...
class IncomeCategoryCreateRequest(BaseModel):
...
class IncomeCategoryListResponse(BaseModel):
...
def _validate_recurrence(
    ...
    if custom_count == 0:
        raise ValueError("Select how often this income repeats")


# ---------------------------------------------------------------------------
# Recurring income — Create / update
# ---------------------------------------------------------------------------
...
class RecurringIncomeCreateRequest(BaseModel):
    income_name: str = Field(..., min_length=1, max_length=255)
    ...
    next_receiving_date: datetime | None = None
    ...
    def validate_fields(self) -> Self:
        # If personal_splits provided, denormalize personal_savings_amount
        ...
        _validate_income_splits(
            self.total_amount,
            self.personal_savings_amount,
            self.family_splits,
            self.personal_splits or None,
        )
        ...
        if self.next_receiving_date is None:
    ...
    def as_form(
        cls,
        income_name: Annotated[str, Form(min_length=1, max_length=255)],
        ...
        received_every: Annotated[str | None, Form()] = None,
        ...
        next_receiving_date: Annotated[str | None, Form()] = None,
        ...
        earned_by_user_id: Annotated[str | None, Form()] = None,
    ) -> Self:
        return cls(
            ...
            received_every=_empty_str_to_none(received_every),
            ...
            next_receiving_date=_parse_optional_datetime(next_receiving_date),
            ...
            earned_by_user_id=_parse_optional_uuid(earned_by_user_id),
        )


class RecurringIncomeQuickAddRequest(BaseModel):
    ...
    def validate_fields(self) -> Self:
        ...
        if self.personal_splits:
            ...
            if fa <= 0 or pa <= 0:
                raise ValueError(
                    "When splitting income, both family and personal amounts must be greater than zero."
                )
...
class RecurringIncomeUpdateRequest(BaseModel):
    income_name: str | None = Field(default=None, min_length=1, max_length=255)
    ...
    def as_form(
        cls,
        income_name: Annotated[str | None, Form(min_length=1, max_length=255)] = None,
        ...
        return cls(
            income_name=_empty_str_to_none(income_name),
...
FamilyRecurringIncomeCreateRequest = RecurringIncomeCreateRequest
FamilyRecurringIncomeUpdateRequest = RecurringIncomeUpdateRequest


# ---------------------------------------------------------------------------
# Recurring income — Response shapes
# ---------------------------------------------------------------------------

class FamilyRecurringIncomeSplitSummary(BaseModel):
    """Read-only family view of one recurring income split."""
...
class FamilyRecurringIncomeSummary(FamilyRecurringIncomeSplitSummary):
...
class RecurringIncomeFamilySplitDetail(BaseModel):
...
class RecurringIncomePersonalSplitDetail(BaseModel):
    user_id: UUID
    amount: Decimal


class RecurringIncomeDetail(BaseModel):
    ...
    doc_viewer_user_ids: list[UUID] = Field(default_factory=list)
    family_splits: list[RecurringIncomeFamilySplitDetail] = Field(default_factory=list)
    personal_splits: list[RecurringIncomePersonalSplitDetail] = Field(default_factory=list)
...
FamilyRecurringIncomeDetail = RecurringIncomeDetail
...
class FamilyIncomeLogCreateRequest(BaseModel):
    """Validated form fields for creating a family income log.

    family_amount and personal_savings_amount are optional — if omitted (and no
    funding_sources), the full total credits this family's savings. If provided,
    ...
    def as_form(
        ...
        income_name: Annotated[str, Form(min_length=1, max_length=255)],
        total_amount: Annotated[Decimal, Form(gt=0)],
        income_date: Annotated[str, Form()],
        earned_by_user_id: Annotated[str | None, Form()] = None,
        family_amount: Annotated[str | None, Form()] = None,
        personal_savings_amount: Annotated[str | None, Form()] = None,
        ...
        return cls(
            income_name=income_name,
            total_amount=total_amount,
            income_date=_parse_optional_datetime(income_date) or datetime.now(timezone.utc),
            earned_by_user_id=_parse_optional_uuid(earned_by_user_id),
            family_amount=_parse_optional_decimal(family_amount),
            personal_savings_amount=_parse_optional_decimal(personal_savings_amount),
...
class FamilyIncomeLogUpdateRequest(BaseModel):
    """Validated form fields for partially updating a family income log.
    ...
    def as_form(
        ...
        total_amount: Annotated[str | None, Form()] = None,
        family_amount: Annotated[str | None, Form()] = None,
        income_date: Annotated[str | None, Form()] = None,
        personal_savings_amount: Annotated[str | None, Form()] = None,
        personal_savings_user_id: Annotated[str | None, Form()] = None,
        earned_by_user_id: Annotated[str | None, Form()] = None,
        document_id: Annotated[str | None, Form()] = None,
        ...
        return cls(
            ...
            total_amount=_parse_optional_decimal(total_amount),
            family_amount=_parse_optional_decimal(family_amount),
            income_date=_parse_optional_datetime(income_date),
            personal_savings_amount=_parse_optional_decimal(personal_savings_amount),
            personal_savings_user_id=_parse_optional_uuid(personal_savings_user_id),
            earned_by_user_id=_parse_optional_uuid(earned_by_user_id),
            funding_sources=_parse_funding_sources(funding_sources),
...
# ---------------------------------------------------------------------------
# Income log — List (paginated, limited fields)
# ---------------------------------------------------------------------------

class FamilyIncomeLogListItem(BaseModel):
    """Limited fields returned in the paginated income log list."""
    ...
    # Joined field — name of the user who earned this income
    earned_by_user_name: str | None = None
    earned_by_user_id: UUID | None = None
    ...
    @computed_field
    def entry_done_by(self) -> UUID:
        return self.added_by_user_id


class FamilyIncomeLogListResponse(PaginatedResponse[FamilyIncomeLogListItem]):
    """Paginated list of family income logs (limited fields)."""


# ---------------------------------------------------------------------------
# Income log — Full detail (for edit modal)
# ---------------------------------------------------------------------------

class FamilyIncomeLogDetailResponse(BaseModel):
    """Full detail of a family income log record (for edit modal)."""
...
# ---------------------------------------------------------------------------
# Income log — Create / Update responses
# ---------------------------------------------------------------------------

class FamilyIncomeLogCreateResponse(BaseModel):
    """Response after successfully creating a family income log."""

    message: str
    log_id: UUID = Field(..., description="ID of the created income log record")
...
class FamilyIncomeLogUpdateResponse(BaseModel):
    """Response after successfully updating a family income log."""

    message: str
    log_id: UUID = Field(..., description="ID of the updated income log record")
...
# ---------------------------------------------------------------------------
# Personal income logs (user personal savings receipts)
# ---------------------------------------------------------------------------

class PersonalIncomeLogListItem(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    ...
    family_id: UUID | None = None
    family_name: str | None = None
    family_income_log_id: UUID | None = None
    document_id: UUID | None = None
    show_doc_to_all: bool = False
    can_edit: bool = False
    category_id: UUID | None = None
...
class PersonalIncomeLogListResponse(PaginatedResponse[PersonalIncomeLogListItem]):
    pass


class PersonalIncomeLogDetailResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    ...
    source_type: str
    source_id: UUID | None
    family_income_log_id: UUID | None
    family_amount: Decimal | None
    document_id: UUID | None
    show_doc_to_all: bool
    can_edit: bool = False
...
class PersonalIncomeLogCreateRequest(BaseModel):
    income_name: str = Field(..., min_length=1, max_length=255)
    amount: Decimal = Field(..., gt=0)
    income_date: datetime
    show_doc_to_all: bool = False
    ...
    def as_form(
        cls,
        income_name: Annotated[str, Form()],
        amount: Annotated[str, Form()],
        income_date: Annotated[str, Form()],
        show_doc_to_all: Annotated[str | None, Form()] = None,
        category_id: Annotated[str | None, Form()] = None,
    ) -> Self:
        parsed_date = _parse_optional_datetime(income_date)
        if parsed_date is None:
            raise ValueError("income_date is required")
...
class PersonalIncomeLogUpdateRequest(BaseModel):
    income_name: str | None = Field(default=None, min_length=1, max_length=255)
    ...
    def as_form(
        cls,
        income_name: Annotated[str | None, Form()] = None,
...
class PersonalIncomeLogCreateResponse(BaseModel):
    message: str
    log_id: UUID
    personal_savings_updated: Decimal


class PersonalIncomeLogUpdateResponse(BaseModel):
    message: str
    log_id: UUID
...
# ---------------------------------------------------------------------------
# Recurring income — List responses
# ---------------------------------------------------------------------------

class FamilyRecurringIncomeSummaryListResponse(PaginatedResponse[FamilyRecurringIncomeSummary]):
    """Paginated summary of all family recurring incomes (visible to every family member)."""


class FamilyRecurringIncomeMineListResponse(PaginatedResponse[FamilyRecurringIncomeDetail]):
    """Paginated full-detail list of incomes added by the current user."""


class FamilyRecurringIncomeCreateResponse(BaseModel):
    """Response after successfully creating a family recurring income."""

    message: str
    income_id: UUID = Field(..., description="ID of the created income record")
...
class FamilyRecurringIncomeUpdateResponse(BaseModel):
    """Response after successfully updating a family recurring income."""

    message: str
    income_id: UUID = Field(..., description="ID of the updated income record")
    document_id: UUID | None = Field(
        default=None,
...
RecurringIncomeMineListResponse = FamilyRecurringIncomeMineListResponse
RecurringIncomeCreateResponse = FamilyRecurringIncomeCreateResponse
RecurringIncomeUpdateResponse = FamilyRecurringIncomeUpdateResponse
